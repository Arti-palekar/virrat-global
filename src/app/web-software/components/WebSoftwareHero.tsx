...
export function WebSoftwareHero() {
  ...
  useEffect(() => {
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          time: { value: 0 },
          color1: { value: new THREE.Color(0xd62020) }, // Virrat Accent Red
          color2: { value: new THREE.Color(0x3b82f6) }, // Accent Blue
          opacity: { value: 0.35 },
        },
        vertexShader: `
          varying vec2 vUv;
    ...
    const createMountains = () => {
      ...
      const layers = [
        { distance: -50, height: 60, color: 0x0f0f10, opacity: 1 },
        { distance: -100, height: 80, color: 0x16161a, opacity: 0.85 },
        { distance: -150, height: 100, color: 0x1e1e24, opacity: 0.7 },
        { distance: -200, height: 120, color: 0x25252e, opacity: 0.5 },
      ];
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            vec3 atmosphere = vec3(0.84, 0.12, 0.12) * intensity;
    ...
    return () => {
      ...
      refs.stars.forEach((sf) => {
        sf.geometry.dispose();
        if (Array.isArray(sf.material)) {
          sf.material.forEach((m) => m.dispose());
        } else {
          sf.material.dispose();
        }
      });
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-screen bg-[#0F0F10] text-white overflow-hidden select-none"
    >
      {/* 3D WebGL Canvas */}
      <canvas
        ref={canvasRef}
      ...
      >
        <div className="flex flex-col gap-1 cursor-pointer">
          <span className="w-6 h-0.5 bg-[#D62020]" />
          <span className="w-4 h-0.5 bg-white" />
          <span className="w-6 h-0.5 bg-white" />
        </div>
        <span className="text-[10px] font-mono tracking-[0.3em] font-bold text-white/70 uppercase">
          VIRRAT SOFTWARE
        </span>
      </div>
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-6">
        ...
        <div
          ref={subtitleRef}
          className="max-w-2xl text-center space-y-2 text-[#A0A0A0] text-base sm:text-xl font-body leading-relaxed"
        >
          <p className="subtitle-line block">
            Engineering Next-Gen Web Applications, SaaS Platforms,
          </p>
          <p className="subtitle-line block text-white font-semibold">
            Custom ERPs, CRMs & AI-Driven Software.
          </p>
        </div>
      </div>
      ...
      <div
        ...
        className="absolute bottom-8 left-8 right-8 z-20 flex items-center justify-between text-xs font-mono text-white/70"
        style={{ visibility: "hidden" }}
      >
        <div className="tracking-[0.2em] uppercase font-bold text-[#D62020]">
          SCROLL TO EXPLORE
        </div>
        <div className="w-32 md:w-64 h-1 bg-white/10 rounded-full overflow-hidden">
          <div
            className="h-full bg-[#D62020] transition-all duration-150 ease-out"
