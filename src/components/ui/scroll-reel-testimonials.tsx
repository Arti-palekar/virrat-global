...
const QUOTE_CLASSES =
  "m-0 text-lg font-medium leading-[1.35] tracking-[-0.02em] text-[#111111] sm:text-[22px] font-heading";
const AUTHOR_CLASSES =
  "m-0 text-sm font-semibold leading-[1.3] text-[#D62020] font-sans";
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-[#ECECEC] bg-gradient-to-b from-white to-[#F5F5F7] blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05)]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-slate-100 ring-2 ring-[#D62020]/20"
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 z-[2] bg-white/10 mix-blend-overlay"
      />
      <div
        aria-hidden="true"
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(214,32,32,0) 32%, rgba(214,32,32,0.3) 41%, rgba(214,32,32,0.5) 47%, rgba(214,32,32,0.2) 54%, rgba(214,32,32,0) 65%)",
        }}
      />
    </div>
  );
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-3xl border border-[#ECECEC] bg-white p-2 shadow-[0_20px_50px_rgba(0,0,0,0.05)] outline-none focus-visible:ring-2 focus-visible:ring-[#D62020] md:min-h-[340px] md:flex-row mx-auto",
        className
      )}
    >
      {/* Reel section */}
      <div
        aria-hidden="true"
        className="relative h-60 w-full shrink-0 self-stretch overflow-hidden rounded-2xl md:h-auto md:w-[380px]"
        style={{
          WebkitMaskImage:
      ...
      {/* Content section */}
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-6 py-8 md:py-12">
        <div className="flex flex-col gap-4">
          <svg
            className="block h-10 w-10 text-[#D62020]/40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
          ...
          >
            <div
              aria-hidden="true"
              className="invisible flex min-h-[130px] flex-col gap-3"
            >
              <p className={QUOTE_CLASSES}>{current.quote}</p>
              <div>
                <p className={AUTHOR_CLASSES}>{current.author}</p>
                {current.role && <span className="text-xs text-[#888888] font-mono">{current.role}</span>}
              </div>
            </div>
            <div
              key={displayIndex}
            ...
            >
              ...
              <div>
                ...
                {current.role && (
                  <span className="text-xs text-[#888888] font-mono block mt-0.5">
                    {current.role}
                  </span>
                )}
              </div>
        ...
        {/* Controls */}
        <div className="mt-8 flex items-center justify-between border-t border-[#F5F5F5] pt-4 md:mt-0">
          <div className="text-xs font-mono text-[#888888]">
            <span className="font-bold text-[#D62020]">0{index + 1}</span> / 0{count}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="grid h-9 w-9 cursor-pointer place-items-center rounded-full border border-[#ECECEC] bg-white text-[#111111] transition-all duration-200 hover:enabled:border-[#D62020] hover:enabled:text-[#D62020] hover:enabled:scale-105 active:enabled:scale-95 disabled:cursor-default disabled:opacity-30 shadow-xs"
            >
              <svg
                className="h-4 w-4"
                viewBox="0 0 12 12"
