...
export function CardStack<T extends CardStackItem>({
  ...
  activeLiftPx = 22,
  activeScale = 1,
  inactiveScale = 0.824, // 280px / 340px approx
  ...
  React.useEffect(() => {
    ...
    if (!len) return;
    // We strictly ignore hover pauses as requested by the user
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-[24px] border-4 border-black/10 dark:border-white/10 overflow-hidden shadow-xl",
                    "select-none",
                    isActive
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          {activeItem.href ? (
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="text-[#D62020] transition"
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  const content = (
    <>
      ...
      {/* solid gradient overlay at bottom for maximum readability */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />

      {/* content */}
      <div className="relative z-10 flex h-full flex-col justify-end p-8 pointer-events-none">
        {/* Optional Icon (e.g. standard icon) */}
        {item.icon && (
          <div className="w-[42px] h-[42px] bg-white rounded-full flex items-center justify-center text-[#D62020] mb-4 shadow-md">
            {item.icon}
          </div>
        )}
        <div className="truncate text-[22px] font-[700] text-white mb-2">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-1 line-clamp-2 text-[15px] font-normal text-white leading-[1.4]">
            {item.description}
          </div>
        ) : null}
      </div>
  ...
  if (item.href) {
    return (
      <Link 
        href={item.href}
        className="block relative h-full w-full bg-white font-syne overflow-hidden rounded-[24px]"
        onClick={(e) => {
          if (!active) {
            e.preventDefault(); // Don't navigate if clicking a side card, just let CardStack make it active
          }
        }}
      >
        {content}
  ...
  return (
    <div className="relative h-full w-full bg-white font-syne overflow-hidden rounded-[24px]">
      {content}
    </div>
  );
}
